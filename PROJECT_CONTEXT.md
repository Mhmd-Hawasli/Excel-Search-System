...
> Update 2026-09-05 (dashboard redesign): `components/app-shell.tsx` now provides a right-side RTL sidebar with four functional sections, active links, persisted desktop collapse (252px → 72px), a 60px mobile icon rail, and an accessible Radix drawer. The centered navbar search submits to `/search`; Ctrl/Cmd+K focuses it, while appearance and logout remain on the left. Shared light/dark tokens and page headings refresh all routes; desktop content uses 85% of the remaining workspace (max 1440px), with smaller responsive margins. The home page now shows archive counts, compact recent files, and existing workflow shortcuts; login and file cards share the new style. Development output uses `.next-dev` to avoid overwriting the running production `.next` build. Also fixed invalid badge nesting on `/edits` and stable drag/drop accessibility IDs on categories. The screenshot index below documents the previous design; current verification images are in `test-artifacts/dashboard-redesign/`.
