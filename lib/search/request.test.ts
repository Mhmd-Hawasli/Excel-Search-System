...
describe("search group parameters", () => {
  ...
  it("uses an empty group list for all groups", () => {
    const result = parseSearchParameters(new URLSearchParams({ q: "أحمد", mode: "custom", field: "full_name" }));
    expect(result.success).toBe(true);
...
describe("search sorting parameters", () => {
  it("accepts a supported column and direction", () => {
    const result = parseSearchParameters(new URLSearchParams({
      q: "أحمد",
      sortBy: "full_name",
      sortDirection: "desc",
    }));
  ...
  it("rejects unsupported columns and directions", () => {
    expect(parseSearchParameters(new URLSearchParams({ q: "أحمد", sortBy: "created_at" })).success).toBe(false);
    expect(parseSearchParameters(new URLSearchParams({ q: "أحمد", sortBy: "source", sortDirection: "sideways" })).success).toBe(false);
  });
});
