...
}).refine((value) => value.mode === "full" || Boolean(value.field), { message: "اختر حقل البحث المخصص." });
...
export function parseSearchParameters(parameters: URLSearchParams) {
  ...
  return searchParametersSchema.safeParse({ ...Object.fromEntries(parameters), groupIds });
}
