...
export function WorkbookSheetSelector({
  ...
  async function selectSheet(sheetName: string) {
    ...
    try {
      const result =
        mode === "linked"
          ? await uploadService.linked(inspection.token, {
              sheetNames: additionalNames,
              nationalIdColumnIndex: Number(nationalColumn),
            })
          : await uploadService.sheet(inspection.token, sheetName);
      onChange(result);
    } catch (cause) {
      setError(cause instanceof ApiError ? cause.message : "تعذر الاتصال؛ حاول مجدداً.");
    } finally {
      onBusyChange(false);
    }
  }
