...
import { formatShamCash } from "@/lib/format";
...
export function UploadWizard({
  ...
  async function goToNextStep() {
    ...
    setNameError(null);
    try {
      const result = await filesService.checkName(trimmedName);
      if (!result.available) {
  ...
  async function confirmImport() {
    ...
    try {
      ...
      toast.error(cause instanceof ApiError ? cause.message : "تعذر بدء الاستيراد.");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function saveTemplate() {
    ...
    try {
      await uploadService.saveTemplate(job.id, templateName);
      setTemplateSaved(true);
      toast.success("تم حفظ قالب الربط.");
    } catch (cause) {
      toast.error(cause instanceof ApiError ? cause.message : "تعذر حفظ القالب.");
    }
  }
  ...
  return (
    <div className="space-y-6">
      <ol className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-5" aria-label="خطوات رفع الملف">
