...
async function requestJson(path: string, method: string, body?: unknown) {
  // V2 transport: same relative /api paths, credentials included, V1 error
  // shape surfaced from the ASP.NET ApiResponse envelope (docs/04).
  ...
  if (!response.ok)
    return { ok: false as const, error: (data as { error?: string } | null)?.error ?? (data as { message?: string } | null)?.message ?? "تعذر تنفيذ العملية." };
      ...
      {catalog.map((group) => (
        <fieldset key={group.key} className="space-y-2.5">
          ...
          {group.key === "groups" ? (
            <>
              {group.permissions
                .filter((permission) => !permission.scoped)
                .map((permission) => {
