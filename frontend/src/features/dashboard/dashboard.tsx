...
export function Dashboard() {
  ...
  const { data, loading, error } = useApiQuery(() => dashboardService.get(), []);

  const permissions = user?.permissions ?? [];
  const canBrowse = user ? canBrowseGroups(permissions) : false;
  const canManageGroups = hasPermission(permissions, "groups.create");
  const canSearch = hasPermission(permissions, "search.view");
  const canUpload = hasPermission(permissions, "upload.view");
  const canExportSection = hasPermission(permissions, "export.view");
  const canMerge = hasPermission(permissions, "merge.view");
  const canSheetMerge = hasPermission(permissions, "sheetMerge.view");
  const canBackup = hasPermission(permissions, "backup.view");

  const values = data ? [data.groupCount, data.fileCount, data.recordCount] : [0, 0, 0];
  const statVisible = [canBrowse, canBrowse, canSearch];
            ...
            <Link href={href} key={label} className="stat-card group">
              <div>
                ...
                <p className="my-2 text-[32px] font-extrabold leading-tight tracking-tight tabular-nums">
                  {loading ? <Skeleton className="h-9 w-20" /> : values[index].toLocaleString("en-US")}
                </p>
            ...
            {error ? (
              ...
              <div>
                ...
                <div className="flex items-center justify-between border-t bg-muted/30 px-5 py-3 text-xs text-muted-foreground">
                  ...
                  <span>{(data?.fileCount ?? 0).toLocaleString("en-US")} ملف في الأرشيف</span>
                </div>
              </div>
            ) : (
        ...
        <div className="dashboard-supplementary space-y-5 max-xl:space-y-0">
          <Card>
            ...
            <CardContent className="p-2 pt-0">
              {quickActions
                .filter((action) => actionAllowed[action.href])
