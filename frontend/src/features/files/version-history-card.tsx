...
export function VersionHistoryCard({ fileId, canExport }: { fileId: string; canExport: boolean }) {
        ...
        <ol className="space-y-3">
          {data.versions.map((entry) => (
            <li key={entry.id} className="flex flex-wrap items-start justify-between gap-2 rounded-lg border p-3">
              ...
              {canExport ? (
                <Button asChild size="sm" variant="outline" title={`تنزيل نسخة الإصدار ${entry.version} محسوبة من سجل التعديلات`}>
