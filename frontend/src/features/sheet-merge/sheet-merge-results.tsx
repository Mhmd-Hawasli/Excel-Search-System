...
function UnlinkedRows({ stat, idColumn }: { stat: SheetMergeSheetResult; idColumn: number }) {
...
function SheetStatCard({ stat, idColumn }: { stat: SheetMergeSheetResult; idColumn: number }) {
...
export function SheetMergeResults({
  ...
  async function exportFile() {
    ...
    try {
      const ready = await sheetMergeService.prepareExport(result.sessionId, (percent, detail) => {
        setExportProgress(Math.min(60, percent));
        if (detail) setExportDetail(detail);
      });
      setExportDetail(`جارٍ تنزيل ${ready.filename}…`);
      await sheetMergeService.download(ready.downloadId, ready.filename, (percent) =>
