...
import {
  sheetMergeService,
  type SheetMergeInspection,
  type SheetMergeRunResult,
} from "@/services/misc.service";

// Appearance ported from V1 features/sheet-merge (interface + results):
// step badges, upload/sheets/key/rules cards, suggestion box, preview table,
// stat boxes, isolated-section footer. Transport stays on sheetMergeService.

/** قاعدة الربط: أكثر من 7 محارف (8 أرقام فأكثر بعد حذف الأصفار البادئة). */
const MIN_NATIONAL_ID_DIGITS = 8;
...
export default function SheetMergePage() {
  const { data: user } = useApiQuery(() => authService.me(), []);

  const [inspection, setInspection] = useState<SheetMergeInspection | null>(null);
  ...
  const [result, setResult] = useState<SheetMergeRunResult | null>(null);
  ...
  if (!hasPermission(user?.permissions ?? [], "sheetMerge.view")) {
    return (
  ...
  async function upload(file: File) {
    if (!/\.xlsx$/i.test(file.name)) {
      setUploadError("الصيغ المقبولة هي XLSX فقط.");
      return;
    }
    setUploading(true);
    setUploadError(null);
    ...
    try {
      const payload = await sheetMergeService.upload(file, (percent, detail) => {
      ...
      // Hidden sheets stay unchecked by default (still checkable by hand).
      setSelectedSheets(
        payload.sheets
          .slice(1)
          .filter((sheet) => sheet.linkable && !sheet.hidden)
  ...
  async function run() {
    ...
    setRunError(null);
    try {
      const payload = await sheetMergeService.run(
