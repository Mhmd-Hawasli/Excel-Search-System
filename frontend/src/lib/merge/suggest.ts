...
/**
 * Imports mapping settings from another table by matching header names.
 *
 * For every source column that has a field mapped, the first target column
 * with an equal normalized header inherits the same field. Each target
 * column is used at most once (first field wins) and the triple full name
 * stays mutually exclusive with the split parts (a copied full name drops
 * copied parts and vice versa).
 */
export function importMappingByHeader(
  sourceHeaders: string[],
  sourceMapping: MergeMapping,
  targetHeaders: string[],
): MergeMapping {
  const normalizedTarget = targetHeaders.map((header) => normalizeStored(header));
  const usedColumns = new Set<number>();
  const mapping: MergeMapping = {};
  for (const field of MERGE_FIELD_KEYS) {
    const sourceIndex = sourceMapping[field];
    if (sourceIndex === undefined) continue;
    const normalizedSource = normalizeStored(sourceHeaders[sourceIndex] ?? "");
    if (!normalizedSource) continue;
    const targetIndex = normalizedTarget.findIndex(
      (header, index) => header === normalizedSource && !usedColumns.has(index),
    );
    if (targetIndex === -1) continue;
    mapping[field] = targetIndex;
    usedColumns.add(targetIndex);
  }

  if (mapping.fullName !== undefined && PART_FIELDS.some((field) => mapping[field] !== undefined)) {
    if (sourceMapping.fullName !== undefined) {
      for (const field of PART_FIELDS) delete mapping[field];
    } else {
      delete mapping.fullName;
    }
