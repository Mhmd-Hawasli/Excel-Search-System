-- Idempotent search indexes. Exact parity with V1 prisma/search-indexes.sql.
-- Run after `dotnet ef database update` when trigram search is enabled.
-- V1 has 14 CREATE INDEX statements; the 3 extra raw-field indexes from the
-- earlier V2 draft are removed (they do not replace normalized indexes).
