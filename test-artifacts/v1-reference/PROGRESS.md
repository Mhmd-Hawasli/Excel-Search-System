...
## قسم معزول: دمج صفحات ملف اكسيل — 2026-09-05

- قسم جديد `/merge-sheets` (رابط `Layers` في القائمة) لدمج عدة صفحات من ملف Excel واحد بالرقم الوطني. معزول كلياً عن الأرشيف: لا Prisma ولا كتابة على القرص — المصنف والجلسة وملف التصدير في ثلاث خرائط `globalThis` بمهلة 12 ساعة (`lib/sheet-merge/store.ts`)، ولا تشارك أي حالة مع قسم «دمج الملفات».
- قواعد الرفع: أكثر من صفحة واحدة (`SHEET_MERGE_MIN_SHEETS`)، وإلغاء عامل التصفية من جميع الأعمدة والصفوف (حذف `autoFilter` وجداول Excel وإظهار الصفوف والأعمدة المخفية) داخل الذاكرة فقط. يُقترح عمود الرقم الوطني في الصفحة الأولى تلقائياً بدمج تطابق العنوان (وزن 0.65) مع نسبة القيم الصالحة في العمود (0.35).
- قاعدة الربط: تُقرأ قيمة الخلية وتُحوَّل إلى رقم (أرقام عربية، فراغات، فواصل آلاف، أصفار بادئة، صيغة علمية) بشرط أكثر من 7 محارف، ومنع تكرار الرقم داخل الصفحة. العمود الأول في كل صفحة إضافية هو مفتاح الربط، وعدد الصفحات غير محدود.
- النتائج: عدد الأسطر ونسبة الربط لكل صفحة، وأسباب التعذر (فارغ / أحرف غير رقمية / أقل من 8 محارف / مكرر / غير موجود في الرئيسية) مع قيم الأسطر كاملة. التصدير `دمج-الصفحات-YYYY-MM-DD.xlsx`: صفحة «الدمج» (أعمدة الأولى ثم أعمدة كل صفحة بدون عمودها الأول) وصفحة «غير مرتبط» لكل صفحة، بتنسيق النظام المعتمد (Light9، RTL، 30pt، تواريخ حقيقية، أسماء أعمدة فريدة).
- نسب مئوية حقيقية للمراحل الثلاث: الرفع (بايتات XHR 0–49% ثم بث NDJSON من الخادم 50–100%)، المعالجة (بث لكل صفحة)، التصدير (تجهيز 0–60% ثم تنزيل محسوب من `content-length`).
- التحقق: 30 اختباراً جديداً، منها `app/api/sheet-merge/flow.test.ts` الذي يقود رفع → دمج → تصدير → تنزيل عبر معالجات HTTP الحقيقية. تشغيل حي على `npm run dev`: الصفحة 200، بث تقدم الرفع (20/35/58/77/95)، ربط 2 من 3 أسطر مع السبب «الرقم 123 طوله 3 محارف»، وملف مُنزَّل 12035 بايت بثلاث صفحات. `prettier --check` نظيف على الملفات الجديدة، و`tsc --noEmit` بلا أخطاء جديدة (349 خطأ قائماً قبل التغيير سببه تعذر `prisma generate` بلا شبكة).

## تنسيق التصدير: Light9 وتوسيط وارتفاع 40px وعرض مناسب — 2026-09-04

- ستايل الجدول `TableStyleLight9` (أزرق فاتح) بلا تخطيط، وكل الخلايا `center + middle` مع التفاف النص، وارتفاع كل الصفوف 30pt (أي 40px)، في ورقتي البيانات والسجل.
- عرض العمود fit-content محسوب من أطول قيمة (العربية بمعامل 1.6)، بحد أدنى 10 وحدات وبسقف 200px (28.57 وحدة).
- التحقق الحي: الأعمدة (10.0 / 22.2 / 28.6) والتواريخ الحقيقية والتمييز الكهرماني، ثم تنظيف البيانات. `typecheck` و`build` ناجحان.

## تحسين التصدير: جدول نظامي وتواريخ حقيقية وعمود الاسم الثلاثي — 2026-09-04

- التصدير (`app/api/files/[id]/export/route.ts`) يبني الآن **جدول Excel حقيقيًا** (Ctrl+T عبر `addTable`): ستايل `TableStyleMedium9` (هيدر كحلي) مع `showRowStripes: false` فتبقى البيانات بيضاء، مع أزرار تصفية، وتمييز كهرماني لرؤوس الأعمدة المعدلة. ورقة «سجل التعديلات» جدول نظامي بالستايل نفسه.
- دالة `parseStoredDate` في `lib/format/date.ts` تحوّل صيغ التخزين (يوم/شهر/سنة، ISO، نص ExcelJS) إلى `Date` حقيقي؛ التصدير يكتبها خلايا تاريخ بتنسيق `DD/MM/YYYY` فيتعرف عليها Excel كتواريخ (31/12/2025). الأرقام الوطنية والهواتف تبقى نصوصًا (الأصفار البادئة محفوظة).
- جدول التعديلات (`/edits` + `listEdits`) يعرض عمود **الاسم الثلاثي** برابط للسجل.
- التحقق: `typecheck` و`build` و174 اختبارًا ناجحة، وفحص حي (جدول `A1:D3` بالستايل المطلوب، تواريخ حقيقية، نصوص محفوظة، تمييز الهيدر، ورقة السجل) ثم تنظيف البيانات.

## التعديل اليدوي على السجلات مع سجل منفصل وتصدير Excel — 2026-09-04

- أضيف جدول `record_edits` (migration `20260904120000_add_record_edits`): كل تعديل يحفظ `record_id` و`file_id` و`file_column_id` و`header_raw` والقيمة القديمة والجديدة والتاريخ، مع علاقات Cascade للحذف و`SetNull` للعمود. أضيف إجراء `RECORD_EDITED` إلى `ActivityAction` وملصقه في `lib/activity.ts`.
- خدمة `lib/edits/service.ts`: حفظ التعديل يحدّث `records.data` ويعيد حساب كل حقول البحث (`sf_*` و`n_*` و`d_*` و`nationalIdNum`) من القيم الحالية، ويعيد بناء مشكلات الجودة للصف (ناقص/خاطئ/هاتف/شام) مع إصلاح مستهدف لتكرار الرقم الوطني للصف المعدل والصفوف المتأثرة بالقيمتين القديمة والجديدة، ثم يسجل `activityLog`. القيمة الأصلية من Excel تبقى مسترجعة دائمًا كأول `oldValue`.
- صفحة السجل: زر قلم لكل حقل مع تحرير inline وحفظ/إلغاء، وشارة «معدّل» كهرمانية على الحقل المعدل مع عرض القيمة الأصلية تحته، وشارة بعدد التعديلات في رأس الصفحة (`components/record-details.tsx` و`app/(protected)/records/[id]/page.tsx`).
- صفحة جديدة «التعديلات والتصدير» (`/edits` + رابط التنقل): بطاقات الملفات المعدلة مع عدد التعديلات وآخر تعديل وزر تصدير Excel وتصفية، وجدول كل التعديلات (الملف/صف Excel/العمود/القديمة/الجديدة/التاريخ) مع ترقيم صفحات.
- تصدير `GET /api/files/[id]/export`: ملف `.xlsx` كامل بالقيم الحالية (الأصل + التعديلات) وتمييز رؤوس الأعمدة المعدلة وورقة ثانية «سجل التعديلات» (صف Excel/العمود/الأصلية/الحالية).
- علامات على مستوى النظام: شارة «معدّل» في `FileCard` (الرئيسية والمجموعة عبر `getEditedFileIds`)، وبانر «هذا الملف تم تعديله يدويًا» مع عدّاد وأزرار السجل والتصدير في صفحة الملف. النسخ الاحتياطي يشمل `recordEdits` تصديرًا واستعادة وتحققًا.
- التحقق: `typecheck` و`build` ناجحان، و174 اختبارًا ناجحًا، وفحص حي على قاعدة البيانات (تعديلان متتاليان يحفظان الأصل، وتحديث البحث، والشارات، والقوائم) ثم تنظيف بيانات الاختبار، وفحص تصدير حي (القيم المعدلة + التمييز + ورقة السجل).

## الدخول من الشبكة دون SESSION_SECRET — 2026-09-02

- أزيل اعتماد التشغيل على `SESSION_SECRET` من التطبيق ومثال البيئة وملف `.env` المحلي. يعتمد تسجيل الدخول على `ADMIN_USERNAME` و`ADMIN_PASSWORD`، مع اشتقاق مفتاح HMAC للجلسة عبر Web Crypto PBKDF2 وتخزينه في ذاكرة العملية. تبقى الجلسات موقعة ومدتها 12 ساعة؛ تغيير بيانات الدخول يبطلها، ويمكن لعدة متصفحات تسجيل الدخول دون تقييد بعنوان IP.
- أصبحت خاصية `Secure` للـcookie مرتبطة باستخدام HTTPS مباشرة أو عبر وكيل، لتعمل جلسة الإنتاج أيضاً عبر HTTP على الشبكة المحلية. يطبق تسجيل الخروج إعدادات الـcookie نفسها. يستمع أمرا `dev` و`start` صراحة على `0.0.0.0`.
- نجحت 174 وحدة اختبار، تشمل الدخول دون السر، رفض كلمة المرور الخاطئة والجلسة المعدلة والمنتهية، إبطال الجلسة بتغيير بيانات الدخول، وHTTP/HTTPS والخروج. نجح التحقق من الأنواع والبناء الإنتاجي. بعد التأكد من عدم وجود مهام استيراد نشطة أعيد تشغيل خادم الإنتاج على 3000.
- نجح الفحص الحي من جهاز الخادم عبر `localhost:3000` و`10.10.20.131:3000`: دخول صحيح، رفض كلمة خاطئة، حماية API قبل الدخول، إتاحة البحث بعده، ومسح cookie عند الخروج. لم يُجر فحص من جهاز ثانٍ، ولم تُعدّل قواعد الجدار الناري أو بيانات الأرشيف. الجلسات الصادرة قبل هذا التعديل تحتاج تسجيل دخول جديد.

## فتح نتيجة البحث في علامة تبويب جديدة — 2026-09-02

- أضيف رمز صغير في أقصى يسار كل نتيجة بحث، كرابط أصلي إلى تفاصيل السجل مع `target="_blank"`؛ يدعم النقر العادي والأوسط وسلوك الروابط المعتاد. يمنع انتشار النقر إلى الصف، ويتجاهل اختصار لوحة مفاتيح الصف الأحداث القادمة من الرابط الداخلي، لتبقى نتائج البحث في الصفحة الحالية.
- عمود الفتح غير قابل للترتيب، وله وصف وصول وتلميح عربي. نجح التحقق من الأنواع وفحص تنسيق الفروق.
