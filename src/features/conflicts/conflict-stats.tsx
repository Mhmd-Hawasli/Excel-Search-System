...
import type { ConflictStats } from "@/lib/conflicts/query";

const RULE_FILTER: Map<string, { category: string; field: string }> = new Map(
  CONFLICT_RULES.map((rule) => [rule.key, { category: rule.category, field: rule.field }]),
);
...
/**
 * Server-rendered statistics dashboard for the conflicts section: archive
 * totals plus bar charts of the most frequent rules and files. Pure
 * presentational — all numbers come precomputed from `queryConflictStats`.
 */
